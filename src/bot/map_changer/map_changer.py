...
class MapChanger:
    ...
    def change_map(cls, from_map: str, to_map: str):
        ...
        sun_x, sun_y = cls._MAP_CHANGE_DATA[from_map][to_map]
        pyag.keyDown("e")
    ...
    def get_shortest_path(cls, start, end) -> dict[str, str]: # {from_map: to_map}
        ...
        if end not in cls._MAP_CHANGE_DATA:
        ...
        while queue:
            ...
            for next_node in cls._MAP_CHANGE_DATA[node]:
                if next_node == end:
                    path += [next_node]
                    return {path[i]: path[i + 1] for i in range(len(path) - 1)}
                if next_node in cls._MAP_CHANGE_DATA and next_node not in visited:
