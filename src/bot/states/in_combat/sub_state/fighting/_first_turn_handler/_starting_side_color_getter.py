...
class Getter:
    ...
    def get_starting_side_color(self, character_pos):
        ...
        closest_cell = self.get_closest_cell(character_pos, starting_cells)
        for map, cell_data in self.__starting_cells.items():
            if map_coords == map:
    ...
    def get_starting_cells(self, map_coords: str):
        starting_cells = []
        for map, cell_data in self.__starting_cells.items():
            if map_coords == map:
    ...
    def get_closest_cell(self, cell_pos, cells_list):
        closest_cell = cells_list[0]
        closest_cell_distance = self.get_distance_between_cells(cell_pos, closest_cell)
        for cell in cells_list:
            distance = self.get_distance_between_cells(cell_pos, cell)
