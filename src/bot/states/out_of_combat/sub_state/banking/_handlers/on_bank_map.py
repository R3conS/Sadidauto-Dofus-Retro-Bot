...
class Handler:
    ...
    def handle(self):
        ...
        result = Vault.open_vault(self._game_window_title)
        if result != Status.SUCCESSFULLY_OPENED_BANK_VAULT:
            return Status.FAILED_TO_FINISH_BANKING

        result = Vault.deposit_all_tabs()
        if result != Status.SUCCESSFULLY_DEPOSITED_ALL_TABS:
            return Status.FAILED_TO_FINISH_BANKING
    
        result = Vault.close_vault()
    ...
    def _enter_astrub_bank(cls):
        ...
        if MapChanger.has_loading_screen_passed():
            if cls._is_char_inside_astrub_bank():
    ...
    def _leave_astrub_bank(cls):
        ...
        pyag.keyUp('e')
        if MapChanger.has_loading_screen_passed():
            if not cls._is_char_inside_astrub_bank():
